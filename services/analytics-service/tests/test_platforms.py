...
class TestEngagementOverTime:
        ...
        fake_db.set_execute_result(
            FakeDBResult(
                rows=[
                    ...
                    FakeRow(
                        period_start="2025-01-02 00:00:00",
                        likes=120,
                        shares=60,
                        comments=40,
                    ),
    ...
    def test_accepts_period_parameter(self, client, fake_db):
        fake_db.set_execute_result(FakeDBResult(rows=[]))
        resp = client.get(
            "/analytics/platforms/engagement-over-time", params={"period": "weekly"}
        )
        assert resp.status_code == 200

    def test_rejects_invalid_period(self, client, fake_db):
        resp = client.get(
            "/analytics/platforms/engagement-over-time", params={"period": "monthly"}
        )
