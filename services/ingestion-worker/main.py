...
async def process_message(message: dict):
    ...
    try:
        ...
        logger.info(
            "fetched_items", platform=platform, count=len(items), tenant_id=tenant_id
        )
        ...
        logger.info(
            ...
            skipped=len(items) - len(saved_items),
            tenant_id=tenant_id,
        )
        ...
        if item_ids:
            await sqs.send_message(
                settings.sqs_ai_pipeline_queue,
                {
                    "tenant_id": tenant_id,
        ...
        logger.error(
            "processing_failed", platform=platform, error=str(e), tenant_id=tenant_id
        )
        await update_worker_status(
            tenant_id,
            worker_run_id,
            ...
            for msg in messages:
                try:
                    ...
                    await sqs.delete_message(
                        settings.sqs_ingestion_queue, msg["ReceiptHandle"]
                    )
                except Exception as e:
